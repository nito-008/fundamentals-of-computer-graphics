...
def getPixelColor(x, y):
    ...
    # 単位法線ベクトルを求める
    n = g_Viewpoint + t * ray - g_Sphere.center
    n = normalize(n)

    # 光線の反射方向ベクトルを求める
    r = g_LightDirection + 2 * np.dot(-g_LightDirection, n) * n
    ...
    if t > 0.0:  # 球との交点がある
        ...
        Id = g_Iin * g_Kd * max(np.dot(-g_LightDirection, n), 0.0)
        Is = g_Iin * g_Ks * max(np.dot(r, -ray), 0.0) ** 5
